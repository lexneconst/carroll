...
int bcd_libmod_init(char *path_dev,  struct mod_struct_t *ext_ptr, struct sysnode_proc_t *str)
{
    ...
    printf("%s:: %s \n", __FUNCTION__, path_dev);
    #endif
    err = pthread_create(&threadid, NULL, &thread_bcdx_procv, (void*) ext_ptr);
	if (err != 0)
 	{
	    //#ifdef DEBUG_H
    ...
    mod_thread_update(stack, threadid, stack_addr, BCDX_CERT_VERIFY_VERSION);
...
int bcd_libmod_exit(struct mod_struct_t *ext_ptr, struct sysnode_proc_t *str)
{
    ...
    if(ext_ptr!=NULL){
        ...
        mod_thread_close(stack, BCDX_CERT_VERIFY_VERSION); 
            //pthread_cancel();
        //}
    }
    libbcd_poweroff();
