...
#define MOD_NAME_STR "map0"

struct mod_desc_t mod_desc = {
    "map",
    "controls procedure map handle",
    "interfaces/map/00",
    0x0000,
    0x0032,
    0x00,
...
int FMOD_GETVER(xmod_getver_params){
    return MAP0_CERT_VERIFY_VERSION;
}
...
int FMOD_INIT(xmod_init_params){
    ...
    do{
        ...
        mod_ext->init   = m10_libmod_init;
        mod_ext->exit   = m10_libmod_exit;
        mod_ext->update = m10_libmod_update;
        mod_ext->find   = m10_libmod_find;
        mod_ext->xread  = m10_libmod_xread;
        mod_ext->xwrite = m10_libmod_xwrite;
        mod_ext->xfree  = m10_libmod_xfree;
...
struct mod_struct_t *FMOD_ALLOC(xmod_alloc_params)
{
    ...
    do{    
        ...
        HANDLE_MOD_INIT(ptr);
        ptr->szdir = libcrt_new_str( szdir);
        //ptr->name = (char *) malloc( 32 );
        //if(ptr->name==NULL){
        //    fatal("allocate memory", __FILE__, __FUNCTION__, __LINE__);
        //    break;        
        //}   
        ptr->name = libcrt_new_str("/map/handle");
    ...
    do{
        ...
        switch(flags){
            ...
            case STACK_VERIFY:
                ...
                if(strcmp(ptr->fdev,MOD_NAME_STR)==0 && verify == MAP0_CERT_VERIFY_VERSION){ 
                    verify = 1;
                }
                *string = (void **) &verify;
            break;
