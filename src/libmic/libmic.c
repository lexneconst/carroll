...
static void capture(char *filename, struct sound_callback *ctx);

static void begin_wave(int ffd, size_t count);
static void end_wave(int ffd);

struct fmt_capture {
	void (*start) (int ffd, size_t count);
	void (*end) (int ffd);
	char *what;
...
int main()
{
    ctx.callback = ctx_callback;
    run("b.wav", &ctx);
    return 0;
}
#else
CRT_FUNCDECL(int, librecord_wave,(sound_callback_threads libthreadproc, struct sysnode_proc_t *psys)){
    ctx.callback = libthreadproc;

    ctx.libmic_init = libmic_init;
    ctx.libmic_exit = libmic_exit;
    ctx.libmic_surx = libmic_surw;
...
int run(char *filename, struct sound_callback *ctx)
{
    capture_stop = 0;
...
/* write a WAVE-header */
static void begin_wave(int ffd, size_t cnt)
{
	WaveHeader h;
	WaveFmtBody f;
...
static void capture(char *orig_name, struct sound_callback *ctx)
{
    ...
    struct sound_struct_t * ptr=NULL;
    uint32_t                fvalue = SOUND_BEGIN;
	...
	do {
		...
		if (fmt_rec_table[file_type].start)
			fmt_rec_table[file_type].start(0, rest);

		/* capture */
		fdcount = 0;

        //pbr = libmic_realloc(pbr);
        //strcpy(pbr->buffer, "xxx");
        //printf("%s", pbr->buffer);
        //printf("\n");
		while (rest > 0 && capture_stop == 0) {
            ...
            if(sound_detection(&ptr, ctx->libmic_surx, audiobuf,c, &fvalue)){
                if(ctx->callback!=NULL){   
                   #ifdef DEBUG_TEST
                   printf("thread callback library %x-%d, %d\r\n", audiobuf, c,  ptr->length);  
                   #endif            
                   ctx->callback(ptr->buffer, ptr->length);
                   fvalue = SOUND_FINAL;
                   if(fvalue == SOUND_FINAL){
                   ...
                   //pbr = libmic_free(pbr);
                    ...
                    fatal("function callback", __FILE__, __FUNCTION__, __LINE__);                
                }
            }else{
                //fatal("detection library", __FILE__, __FUNCTION__, __LINE__);            
            }
