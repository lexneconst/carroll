...
struct bthost *bthost_create(void)
{
 ...
 void l2conn_free(struct l2conn *conn)
{
	free(conn);
}
 ...
 void btconn_free(struct btconn *conn)
{
 ...
 struct btconn *bthost_find_conn(struct bthost *bthost, uint16_t handle)
{
	struct btconn *conn;
 ...
 void bthost_add_l2cap_conn(struct bthost *bthost, struct btconn *conn,
						uint16_t scid, uint16_t dcid)
{
 ...
 struct l2conn *btconn_find_l2cap_conn_by_scid(struct btconn *conn,
								uint16_t scid)
{
	struct l2conn *l2conn;
							...
							void *user_data)
{
 ...
 void queue_command(struct bthost *bthost, const void *data,
								uint16_t len)
{
 ...
 void send_packet(struct bthost *bthost, const void *data, uint16_t len)
{
 ...
 void send_acl(struct bthost *bthost, uint16_t handle, uint16_t cid,
						const void *data, uint16_t len)
{
 ...
 uint8_t _l2cap_sig_send(struct bthost *bthost, struct btconn *conn,
					uint8_t code, uint8_t ident,
					const void *data, uint16_t len)
{
	 uint8_t next_ident = 1;
				...
				bthost_l2cap_rsp_cb cb, void *user_data)
{
	...
	ident = _l2cap_sig_send(bthost, conn, code, 0, data, len);
	if (!ident)
		return false;
 ...
 void send_command(struct bthost *bthost, uint16_t opcode,
						const void *data, uint8_t len)
{
 ...
 void next_cmd(struct bthost *bthost)
{
	struct cmd_queue *cmd_q = &bthost->cmd_q;
 ...
 void read_bd_addr_complete(struct bthost *bthost, const void *data,
								uint8_t len)
{
 ...
 void _evt_cmd_complete(struct bthost *bthost, const void *data,
								uint8_t len)
{
 ...
 void _evt_cmd_status(struct bthost *bthost, const void *data,
								uint8_t len)
{
 ...
 void _evt_conn_request(struct bthost *bthost, const void *data,
								uint8_t len)
{
 ...
 void init_conn(struct bthost *bthost, uint16_t handle, uint8_t addr_type)
{
	struct btconn *conn;
 ...
 void _evt_conn_complete(struct bthost *bthost, const void *data,
								uint8_t len)
{
 ...
 void _evt_disconn_complete(struct bthost *bthost, const void *data,
								uint8_t len)
{
 ...
 void _evt_num_completed_packets(struct bthost *bthost, const void *data,
								uint8_t len)
{
 ...
 void evt_le_conn_complete(struct bthost *bthost, const void *data,
								uint8_t len)
{
 ...
 void _evt_le_meta_event(struct bthost *bthost, const void *data,
								uint8_t len)
{
 ...
 void process_evt(struct bthost *bthost, const void *data, uint16_t len)
{
	...
	case BT_HCI_EVT_CMD_COMPLETE:
		_evt_cmd_complete(bthost, param, hdr->plen);
		break;

	case BT_HCI_EVT_CMD_STATUS:
		_evt_cmd_status(bthost, param, hdr->plen);
		break;

	case BT_HCI_EVT_CONN_REQUEST:
		_evt_conn_request(bthost, param, hdr->plen);
		break;

	case BT_HCI_EVT_CONN_COMPLETE:
		_evt_conn_complete(bthost, param, hdr->plen);
		break;

	case BT_HCI_EVT_DISCONNECT_COMPLETE:
		_evt_disconn_complete(bthost, param, hdr->plen);
		break;

	case BT_HCI_EVT_NUM_COMPLETED_PACKETS:
		_evt_num_completed_packets(bthost, param, hdr->plen);
		break;

	case BT_HCI_EVT_LE_META_EVENT:
		_evt_le_meta_event(bthost, param, hdr->plen);
		break;
 ...
 bool _l2cap_cmd_rej(struct bthost *bthost, struct btconn *conn,
				uint8_t ident, const void *data, uint16_t len)
{
 ...
 bool _l2cap_conn_req(struct bthost *bthost, struct btconn *conn,
				uint8_t ident, const void *data, uint16_t len)
{
	...
	_l2cap_sig_send(bthost, conn, BT_L2CAP_PDU_CONN_RSP, ident, &rsp,
								sizeof(rsp));
	...
	if (!rsp.result) {
		...
		_l2cap_sig_send(bthost, conn, BT_L2CAP_PDU_CONFIG_REQ, 0,
 ...
 bool _l2cap_conn_rsp(struct bthost *bthost, struct btconn *conn,
				uint8_t ident, const void *data, uint16_t len)
{
 ...
 bool _l2cap_config_req(struct bthost *bthost, struct btconn *conn,
				uint8_t ident, const void *data, uint16_t len)
{
	...
	_l2cap_sig_send(bthost, conn, BT_L2CAP_PDU_CONFIG_RSP, ident, &rsp,
								sizeof(rsp));

	return true;
}

 bool _l2cap_config_rsp(struct bthost *bthost, struct btconn *conn,
				uint8_t ident, const void *data, uint16_t len)
{
 ...
 bool _l2cap_disconn_req(struct bthost *bthost, struct btconn *conn,
				uint8_t ident, const void *data, uint16_t len)
{
	...
	_l2cap_sig_send(bthost, conn, BT_L2CAP_PDU_DISCONN_RSP, ident, &rsp,
								sizeof(rsp));

	return true;
}

 bool _l2cap_info_req(struct bthost *bthost, struct btconn *conn,
				uint8_t ident, const void *data, uint16_t len)
{
	...
	_l2cap_sig_send(bthost, conn, BT_L2CAP_PDU_INFO_RSP, ident, &rsp,
								sizeof(rsp));

	return true;
}

 void handle_pending_l2reqs(struct bthost *bthost, struct btconn *conn,
						...
						const void *data, uint16_t len)
{
 ...
 void _l2cap_sig(struct bthost *bthost, struct btconn *conn,
						const void *data, uint16_t len)
{
	...
	case BT_L2CAP_PDU_CMD_REJECT:
		ret = _l2cap_cmd_rej(bthost, conn, hdr->ident,
						data + sizeof(*hdr), hdr_len);
		break;

	case BT_L2CAP_PDU_CONN_REQ:
		ret = _l2cap_conn_req(bthost, conn, hdr->ident,
						data + sizeof(*hdr), hdr_len);
		break;

	case BT_L2CAP_PDU_CONN_RSP:
		ret = _l2cap_conn_rsp(bthost, conn, hdr->ident,
						data + sizeof(*hdr), hdr_len);
		break;

	case BT_L2CAP_PDU_CONFIG_REQ:
		ret = _l2cap_config_req(bthost, conn, hdr->ident,
						data + sizeof(*hdr), hdr_len);
		break;

	case BT_L2CAP_PDU_CONFIG_RSP:
		ret = _l2cap_config_rsp(bthost, conn, hdr->ident,
						data + sizeof(*hdr), hdr_len);
		break;

	case BT_L2CAP_PDU_DISCONN_REQ:
		ret = _l2cap_disconn_req(bthost, conn, hdr->ident,
						data + sizeof(*hdr), hdr_len);
		break;

	case BT_L2CAP_PDU_INFO_REQ:
		ret = _l2cap_info_req(bthost, conn, hdr->ident,
						data + sizeof(*hdr), hdr_len);
		break;
...
reject:
	...
	_l2cap_sig_send(bthost, conn, BT_L2CAP_PDU_CMD_REJECT, 0,
							&rej, sizeof(rej));
}

 bool _l2cap_conn_param_req(struct bthost *bthost, struct btconn *conn,
				uint8_t ident, const void *data, uint16_t len)
{
	...
	memset(&rsp, 0, sizeof(rsp));
	_l2cap_sig_send(bthost, conn, BT_L2CAP_PDU_CONN_PARAM_RSP, ident,
							&rsp, sizeof(rsp));

	return true;
}

 bool _l2cap_conn_param_rsp(struct bthost *bthost, struct btconn *conn,
				uint8_t ident, const void *data, uint16_t len)
{
 ...
 void _l2cap_le_sig(struct bthost *bthost, struct btconn *conn,
						const void *data, uint16_t len)
{
	const struct bt_l2cap_hdr_sig *hdr = data;
	struct bt_l2cap_pdu_cmd_reject rej;
	...
	case BT_L2CAP_PDU_CONN_PARAM_REQ:
		ret = _l2cap_conn_param_req(bthost, conn, hdr->ident,
						data + sizeof(*hdr), hdr_len);
		break;

	case BT_L2CAP_PDU_CONN_PARAM_RSP:
		ret = _l2cap_conn_param_rsp(bthost, conn, hdr->ident,
						data + sizeof(*hdr), hdr_len);
		break;
...
reject:
 ...
 void process_acl(struct bthost *bthost, const void *data, uint16_t len)
{
	...
	switch (cid) {
	case 0x0001:
		_l2cap_sig(bthost, conn, l2_data, l2_len);
		break;
	case 0x0005:
		_l2cap_le_sig(bthost, conn, l2_data, l2_len);
		break;
	default:
