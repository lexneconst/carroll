#include <libcrt.h>
 ...
 void cprint_dev_info(int ctl, struct hci_dev_info *di);

 void cprint_dev_list(int ctl, int flags)
{
...
*/
 void cmd_le_addr(int ctl, int hdev, char *opt)
{
 ...
 void cmd_le_adv(int ctl, int hdev, char *opt)
{
	struct hci_request rq;
 ...
 void cmd_no_le_adv(int ctl, int hdev, char *opt)
{
	struct hci_request rq;
	le_set_advertise_enable_cp advertise_cp;
	uint8_t status;
 ...
 void cmd_le_states(int ctl, int hdev, char *opt)
{
 ...
 void cmd_features(int ctl, int hdev, char *opt)
{
 ...
 void cmd_cname(int ctl, int hdev, char *opt)
{
	int dd;
 ...
 */
 ...
 char *cget_minor_device_name(int major, int minor)
{
	switch (major) {
	case 0:	/* misc */
		return "";
...
{
	 const char *services[] = { "Positioning",
					"Networking",
					"Rendering",
					"Capturing",
					"Object Transfer",
					...
					"Telephony",
					"Information" };
	 const char *major_devices[] = { "Miscellaneous",
					"Computer",
					"Phone",
					"LAN Access",
					"Audio/Video",
	...
	if (opt) {
		...
		if ((cls[1] & 0x1f) >= sizeof(major_devices) / sizeof(*major_devices))
			...
			printf("%s, %s\n", major_devices[cls[1] & 0x1f],
				cget_minor_device_name(cls[1] & 0x1f, cls[0] >> 2));
	}
}
 ...
 void cmd_voice(int ctl, int hdev, char *opt)
{
 ...
 void cmd_delkey(int ctl, int hdev, char *opt)
{
	bdaddr_t bdaddr;
	uint8_t all;
	int dd;
 ...
 void cmd_oob_data(int ctl, int hdev, char *opt)
{
 ...
 void cmd_commands(int ctl, int hdev, char *opt)
{
	uint8_t cmds[64];
	char *str;
	int i, n, dd;
 ...
 void cmd_version(int ctl, int hdev, char *opt)
{
 ...
 void cmd_inq_tpl(int ctl, int hdev, char *opt)
{
	int dd;
 ...
 void cmd_inq_mode(int ctl, int hdev, char *opt)
{
 ...
 void cmd_inq_data(int ctl, int hdev, char *opt)
{
	int i, dd;
 ...
 void cmd_inq_type(int ctl, int hdev, char *opt)
{
	int dd;
 ...
 void cmd_afh_mode(int ctl, int hdev, char *opt)
{
 ...
 void cmd_ssp_mode(int ctl, int hdev, char *opt)
{
	int dd;
 ...
 void print_rev_ericsson(int dd)
{
	struct hci_request rq;
...
/*
 void print_rev_csr(int dd, uint16_t rev)
{
...
*/
 void print_rev_digianswer(int dd)
{
	struct hci_request rq;
 ...
 void print_rev_broadcom(uint16_t hci_rev, uint16_t lmp_subver)
{
	printf("\tFirmware %d.%d / %d\n",
		hci_rev & 0xff, lmp_subver >> 8, lmp_subver & 0xff);
}

 void print_rev_avm(uint16_t hci_rev, uint16_t lmp_subver)
{
	if (lmp_subver == 0x01)
 ...
 void cmd_revision(int ctl, int hdev, char *opt)
{
	struct hci_version ver;
	int dd;
 ...
 void cmd_block(int ctl, int hdev, char *opt)
{
	bdaddr_t bdaddr;
	int dd;
 ...
 void cmd_unblock(int ctl, int hdev, char *opt)
{
	bdaddr_t bdaddr;
	int dd;
...
void hci_up(struct libvbt_t *st)
{
    int ctl = -1;
    int dev_id = -1;
    bdaddr_t bdaddr;
    ...
    printf("address (%s) - %d \n", st->addr, dev_id);
    cmd_up(ctl, dev_id, NULL);
    
    close(ctl);
}

void hci_down(struct libvbt_t *st)
{
    int ctl = -1;
    int dev_id = -1;
    bdaddr_t bdaddr;
